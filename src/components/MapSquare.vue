<template>
  <div
    class="map-square ground"
    :class="getMapSquareClass()"
    @click="doMapSquareClick"
  >
    <civilization-tile
      v-if="hasTile && !tile.isLeaderTile"
    ...
    <monument-tile
      v-if="showMonument"
      :monument-type="tile.monumentType"
      class="monument"
    />
    <div
      v-if="isRiverTile && riverPath === '='"
      class="river river-horizontal"
    />
    <div
      v-if="isRiverTile && riverPath === '║'"
      class="river river-vertical"
    />
    <div
      v-if="isRiverTile && showRiverHorizontalLeft"
      class="river river-horizontal-left"
    />
    <div
      v-if="isRiverTile && showRiverHorizontalRight"
      class="river river-horizontal-right"
    />
    <div
      v-if="isRiverTile && showRiverVerticalBottom"
      class="river river-vertical-bottom"
    />
    <div
      v-if="isRiverTile && showRiverVerticalTop"
      class="river river-vertical-top"
    />
    <div
      v-if="showKingdoms"
      class="kingdom"
...
const hasTile = computed(() => props.tile && props.tile.tileType !== tileTypes.empty)

const showRiverHorizontalLeft = computed(() => riverPath.value === '╗' || riverPath.value === '╝')

const showRiverHorizontalRight = computed(() => riverPath.value === '╔' || riverPath.value === '╚')

const showRiverVerticalBottom = computed(() => riverPath.value === '╔' || riverPath.value === '╗')

const showRiverVerticalTop = computed(() => riverPath.value === '╚' || riverPath.value === '╝')
...
const kingdomStyle = computed(() => {
  ...
  return kingdomIndex !== null && kingdomIndex >= 0
    ? `background-color: #${colors[kingdomIndex]};`
    : blankCss
})

const showMonument = computed(
  () =>
    props.tile && props.tile.monumentType && props.tile.tileType === tileTypes.monumentBottomRight
)
...
const isConflictTile = computed(
  () =>
    !!(conflictTile.value && props.index !== undefined && conflictTile.value.index === props.index)
)
...
function getMapSquareClass(): string {
  ...
  if (
    Array.isArray(availableTileLocations) &&
