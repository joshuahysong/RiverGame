...
const helpers = {
  getTileNameByType(tileType: number): string {
    return pascalToKebab(Object.keys(tileTypes).find(key => tileTypes[key as keyof typeof tileTypes] === tileType) || '')
  },
  ...
  getActionNameByType(actionTypeId: number): string | undefined {
    return Object.keys(actionTypes).find(key => actionTypes[key as keyof typeof actionTypes] === actionTypeId)
  },
  
  getMessageNameByType(messageType: number): string {
    return pascalToKebab(Object.keys(messageTypes).find(key => messageTypes[key as keyof typeof messageTypes] === messageType) || '')
  },
  ...
  getMonumentNameByType(monumentType: number): string {
    return this.capitalizeFirstLetter(
      pascalToProper(Object.keys(monumentTypes).find(key => monumentTypes[key as keyof typeof monumentTypes] === monumentType) || '')
    ).replace(' ', ' & ')
  },
